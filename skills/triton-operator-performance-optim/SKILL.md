...
description: 优化 Ascend NPU 亲和的 Triton 算子性能。当用户需要优化 Triton 算子在昇腾 NPU 上的性能、解决 UB 溢出、提高 Cube 单元利用率、进行 Tiling 策略设计时使用。
---
...
### Phase 5: AscendC 中的优化经验参考

根据 Phase 1 和 Phase 2 所分析的性能瓶颈描述，使用 `script/get_ascendc_optimization_advice.py` 获取 AscendC 中的优化经验参考， 返回最相关的 1 条。使用方法如下

```python
python script/get_ascendc_optimization_advice.py --bottleneck "Maximizing UB batching for multi-row fusion"
```
- bottleneck：性能瓶颈描述，例如 "Maximizing UB batching for multi-row fusion"

返回包含: title, description, bottleneck, code_diff，其中 code_diff 是 ascend c 代码的 code diff 格式，是一个 c extension 的代码的优化前后的改动，作为 triton kernel 的优化参考。

### Phase 6: 验证（MANDATORY）
