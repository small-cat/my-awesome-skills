...
description: 根据算子设计文档生成 Ascend NPU 的 Triton kernel 代码。当用户需要实现 Triton 算子 kernel、将需求文档转化为可执行代码时使用。核心能力：(1)解析需求文档确认计算逻辑 (2)设计 tiling 分块策略 (3)生成高性能 kernel 代码 (4)生成测试代码验证正确性。
---
...
**核间切分**：grid 大小优先设置为物理核数，通过核内任务分块循环实现完整计算。
