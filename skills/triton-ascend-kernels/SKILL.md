...
```shell
python -m <op_name>.test_<op_name>_verify
```
to verify correctness and precision. Then run the performance test:
```shell
msprof op --output=<output_dir> --kernel-name="<op_name>_kernel" python -m <op_name>.test_<op_name>_perf
```
...
- Performance test (vs baseline): `msprof op --output=<user-specified-path> --kernel-name="<op_name>_kernel" python -m <op_name>.test_<op_name>_perf`
- Correctness verification: `python -m <op_name>.test_<op_name>_verify`
